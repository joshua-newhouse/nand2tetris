...
void WritePushPop(Parser_command *cmd, char *out, char *fileName, int nLine);
void WriteArithmetic(Parser_command *cmd, char *out, int nLine);
int Type(Parser_command *cmd, enum type t);
void WriteSymLab(char *out, const char *s, int x, enum symLab sl, int nLine);
char *GenSym(char *arg2, char *fileName);
...
char *CW_WriteCode(Parser_command *cmd, char *output, FILE *fp, FH_FileStruct inFile){
	*output = '\0';
	...
	case C_PUSH:
	case C_POP:
		WritePushPop(cmd, output, inFile.fileName, inFile.nLine);
		break;
	case C_ARITHMETIC:
		WriteArithmetic(cmd, output, inFile.nLine);
		break;
	default:
		break;
	}
...
void WritePushPop(Parser_command *cmd, char *out, char *fileName, int nLine){
	...
	case LOCAL:
		...
		break;
	case STATIC:
		if((s = GenSym(cmd->arg2, fileName)) == NULL){
			errCMD(nLine, fileName, "Cannot allocate memory for symbol");
		}
		if(isPush){
			strcat(out, "@");
			strcat(out, s);
	...
	case CONSTANT:
		if(isPush){
			...
			errCMD(nLine, fileName, "Cannot pop constant");
		break;
	case THIS:
		if(isPush){
			GetValAtAddr(THIS);
	...
	case POINTER:
		...
		case 1:
			...
			break;
		default:
			errCMD(nLine, fileName, "Pointer out of bounds");
		}
		if(isPush){
			strcat(out, s);
			strcat(out, "D=M\n");
...
void WriteArithmetic(Parser_command *cmd, char *out, int nLine){
	...
	case EQ:
		...
		SP_DEC(out);
		strcat(out, "D=M-D\n");
		WriteSymLab(out, "EQ", eqCond, SYMBOL, nLine);
		strcat(out, "D;JEQ\n");
		strcat(out, SET_FALSE);
		WriteSymLab(out, "CONTINUE_EQ", eqCond, SYMBOL, nLine);
		strcat(out, "0;JMP\n");
		WriteSymLab(out, "EQ", eqCond, LABEL, nLine);
		strcat(out, SET_TRUE);
		WriteSymLab(out, "CONTINUE_EQ", eqCond, LABEL, nLine);
		SP_INC(out);
		break;
	case GT:
		gtCond++;
		...
		SP_DEC(out);
		strcat(out, "D=M-D\n");
		WriteSymLab(out, "GT", gtCond, SYMBOL, nLine);
		strcat(out, "D;JGT\n");
		strcat(out, SET_FALSE);
		WriteSymLab(out, "CONTINUE_GT", gtCond, SYMBOL, nLine);
		strcat(out, "0;JMP\n");
		WriteSymLab(out, "GT", gtCond, LABEL, nLine);
		strcat(out, SET_TRUE);
		WriteSymLab(out, "CONTINUE_GT", gtCond, LABEL, nLine);
		SP_INC(out);
		break;
	case LT:
		ltCond++;
		...
		SP_DEC(out);
		strcat(out, "D=M-D\n");
		WriteSymLab(out, "LT", ltCond, SYMBOL, nLine);
		strcat(out, "D;JLT\n");
		strcat(out, SET_FALSE);
		WriteSymLab(out, "CONTINUE_LT", ltCond, SYMBOL, nLine);
		strcat(out, "0;JMP\n");
		WriteSymLab(out, "LT", ltCond, LABEL, nLine);
		strcat(out, SET_TRUE);
		WriteSymLab(out, "CONTINUE_LT", ltCond, LABEL, nLine);
		SP_INC(out);
		break;
	case AND:
		SP_DEC(out);
...
char *GenSym(char *arg2, char *fileName){
	char *t = (char *)malloc(strlen(fileName) + 2 + strlen(arg2));
	*t = '\0';
	strcat(t, fileName);
	strcat(t, ".");
	strcat(t, arg2);
